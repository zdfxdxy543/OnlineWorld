...
class SQLiteNewsRepository(AbstractNewsRepository):
    ...
    def list_articles(self, category: str | None = None, limit: int = 20) -> list[ArticleSummary]:
        query = """
        SELECT
            id, title, category, author_id, published_at, views, is_pinned, excerpt
        FROM news_articles
        """
        params = []
    ...
    def get_article(self, article_id: str) -> ArticleDetail | None:
        with self.session_manager.connect() as conn:
            article_row = conn.execute(
                """
                SELECT
                    id, title, category, author_id, published_at, views, is_pinned, excerpt, content
                FROM news_articles
                WHERE id = ?
                """,
                (article_id,),
        ...
        *,
        ...
        with self.session_manager.connect() as conn:
            conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (article_id, title, content, excerpt, category, author_id, now, 0, 1 if is_pinned else 0),
            )
